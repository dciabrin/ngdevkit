...
def register_nss_ops():
    nss_opcodes = (
        ...
        ("s_pitch" , ["pitch"]),
        # 0x40
        ("fm2_op1_note_on", ["note"]),
        ("fm2_op2_note_on", ["note"]),
        ("fm2_op3_note_on", ["note"]),
        ("fm2_op4_note_on", ["note"]),
        ("set_2ch", ),
        ("fm_cut",   ["delay"]),
        ("s_cut",    ["delay"]),
        ("a_cut",    ["delay"]),
        ...
        ("legato",      ),
        ("legato_off",  ),
        ("fm2_vibrato", ["op", "speed_depth"]),
        ("fm2_vibrato_off", ["op"]),
        ("fm2_note_porta",  ["op", "speed"]),
        ("fm2_legato"   , ["op"]),
        ("fm2_legato_off", ["op"]),
        ("fm2_vol_slide_off", ["op"]),
        ("fm2_vol_slide_u"  , ["op", "increment"]),
        ("fm2_vol_slide_d"  , ["op", "increment"]),
...
class channel_nss_factory:
    ...
    def ext_fm_factory(channel):
        ext_instr = {0: fm2_op1_instr, 1: fm2_op2_instr, 2: fm2_op3_instr, 3: fm2_op4_instr}
        ext_note = {0: fm2_op1_note_on, 1: fm2_op2_note_on, 2: fm2_op3_note_on, 3: fm2_op4_note_on}
        ext_vol = {0: fm2_op1_vol, 1: fm2_op2_vol, 2: fm2_op3_vol, 3: fm2_op4_vol}
        def stop_op_lambda():
            # bitmask 7..4: op4|op3|op2|op1,  0 for stopped channel, 1 otherwise
            bitmask = ~(1 << (channel+4)) & 0xff
            return fm2_ops_off(bitmask)

        return channel_nss_factory("O%d"%channel,
                                   fm_ctx_2, ext_instr[channel], ext_vol[channel], 0x7f,
                                   fm_pan, fm_pitch, ext_note[channel], stop_op_lambda,
                                   channel_nss_factory.mk_no_impl("retrigger"), fm_cut, fm2_delay)
...
def furnace_row_to_nss_actions(fur_pattern, pos):
    ...
    def mkfx(fxname, *args):
        """instantiate an FX for a channel or a FM2 extended OP"""
        if ext_fm2 and location_channel in [1,2,3,4]:
            fxname = globals()["fm2_"+fxname.__name__]
            op_ym2610_order = [0, 0, 2, 1, 3][location_channel]
            args = (op_ym2610_order,)+args
        return fxname(*args)
    ...
    location_pos = (4,2)
    if row.ins != -1:
        out.ins.append(factory.instr(row.ins))
    ...
    for fx, fxval in row.fx:
        ...
        # arpeggio
            ...
            if fxval in [-1, 0]:
                ...
                out.pre_fx.append(mkfx(arpeggio_off))
            else:
                out.fx.append(mkfx(arpeggio, fxval))
        # pitch slide up
        elif fx == 0x01:
            if fxval in [-1, 0]:
                # opcode must be executed before a note opcode
                out.pre_fx.append(mkfx(note_slide_off))
            else:
                out.fx.append(mkfx(note_pitch_slide_u, fxval))
        # pitch slide down
        elif fx == 0x02:
            if fxval in [-1, 0]:
                # opcode must be executed before a note opcode
                ...
                out.fx.append(mkfx(note_pitch_slide_d, fxval))
        # portamento
        elif fx == 0x03:
            out.fx.append(mkfx(note_porta, fxval))
        # vibrato
        elif fx == 0x04:
            if fxval in [-1, 0]:
                # opcode must be executed before a note opcode
                out.pre_fx.append(mkfx(vibrato_off))
            else:
                out.fx.append(mkfx(vibrato, fxval))
        # panning
        elif fx == 0x08:
        ...
        # volume slide up/down
            ...
            if fxval in [-1, 0]:
                ...
                out.pre_fx.append(mkfx(vol_slide_off))
                print(out.pre_fx)
            elif fxval > 0x0f:
                out.fx.append(mkfx(vol_slide_u, fxval>>4))
            else:
                out.fx.append(mkfx(vol_slide_d, fxval))
        # retrigger
        elif fx == 0x0c:
        ...
        # arpeggio speed
            ...
            fxval = max(fxval, 1)
            out.fx.append(mkfx(arpeggio_speed, fxval))
        # slide up
        elif fx == 0xe1:
            if fxval in [-1, 0]:
                # opcode must be executed before a note opcode
                ...
                # opcode must be executed before a note opcode
                out.post_fx.append(mkfx(note_slide_u, fxval))
        # slide down
        elif fx == 0xe2:
            if fxval in [-1, 0]:
                out.pre_fx.append(mkfx(note_slide_off))
            else:
                out.post_fx.append(mkfx(note_slide_d, fxval))
        # pitch
        elif fx == 0xe5:
        ...
        # quick legato up/down
            ...
            if 8 <= ticks <= 15:
                out.fx.append(mkfx(quick_legato_d, (ticks-8)<<4|semitones))
            else:
                out.fx.append(mkfx(quick_legato_u, (ticks)<<4|semitones))
        # quick legato up
        elif fx == 0xe8:
            out.fx.append(mkfx(quick_legato_u, fxval))
        # quick legato down
        elif fx == 0xe9:
            out.fx.append(mkfx(quick_legato_d, fxval))
        # legato
        elif fx == 0xea:
            if fxval in [-1, 0]:
                # opcode must be executed before a note opcode
                out.pre_fx.append(mkfx(legato_off))
            else:
                # opcode must be executed before a note opcode
                out.pre_fx.append(mkfx(legato))
        # note cut
        elif fx == 0xec:
            out.fx.append(mkfx(factory.cut, fxval))
        # note delay
        elif fx == 0xed:
            # opcode must be executed before a note opcode
            out.pre_fx.append(mkfx(factory.delay, fxval))
        # stop song
        elif fx == 0xff:
...
def merge_extended_f2_patterns(nss_orders, nss_patterns):
    def row_fm2_from_op1234(o1, o2, o3, o4):
        ...
        # merge all instr
        fm2.ins= o1.ins + o2.ins + o3.ins + o4.ins
        # merge all post fx
        ...
        stopped_ops=sum([~o.ops&0xff for o in notes if isinstance(o, fm2_ops_off)])
        if stopped_ops == 0b11110000:
            fm2.note.append(fm_stop())
        elif stopped_ops != 0:
            fm2.note.append(fm2_ops_off(~stopped_ops&0xff))
        # merged_stopped_ops
        played_notes=[o for o in notes if o != [] and not isinstance(o, fm2_ops_off)]
...
def compact_instr(nss, **kwargs):
    ...
    fm2_op_map = {fm2_op1_instr: 0, fm2_op2_instr: 1, fm2_op3_instr: 2, fm2_op4_instr: 3}
    fm_ctx_map = {fm_ctx_1: 0, fm_ctx_2: 1, fm_ctx_3: 2, fm_ctx_4: 3}
    fm_ctx = 0
    s_ctx_map = {s_ctx_1: 0, s_ctx_2: 1, s_ctx_3: 2}
    s_ctx = 0
    ...
    def compact_instr_pass(op, out):
        ...
        if type(op) == nss_label:
            ...
            if fm_is[fm_ctx] != op.inst:
                fm_is[fm_ctx] = op.inst
                out.append(op)
        elif type(op) in fm2_op_map.keys():
            oidx = fm2_op_map[type(op)]
            if op_is[oidx] != op.inst:
                op_is[oidx] = op.inst
                out.append(op)
        elif type(op) in fm_ctx_map.keys():
            fm_ctx = fm_ctx_map[type(op)]
...
def main():
    ...
    # ext_fm2: consider all OPs if channel 1 is selected
    if ext_fm2 and '1' in arguments.channels:
        channels = ''.join(sorted(set(channels+'wxyz')))
    ...
    if ext_fm2 and next((True for c in channels if c>=14), False):
        channels = sorted(set(channels+[1]))
